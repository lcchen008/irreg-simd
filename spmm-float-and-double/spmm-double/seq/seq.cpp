...
int main(int argc, char** argv) {
  ...
  Csr<double>* m2 = CsrLoader<double>::Load(argv[1]); 
  cout << "m2 load done." << endl;
  cout << "Total rows: " << m1->num_rows << endl;
  vector<FastHash<int, double >* > row_results(m1->num_rows);
  for (auto& v : row_results) {
    v = new FastHash<int, double>(atoi(argv[2]));
  }

  // Do the SpMM computation.
  Csr<double>* res = SpMM(m1, m2, row_results); 
