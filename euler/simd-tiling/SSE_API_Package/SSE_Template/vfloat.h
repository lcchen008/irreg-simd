...
public:
    ...
    //load continuous memory space (unaligned)
    void loadu(void *src){
        val = _mm512_loadunpacklo_ps(val, src);
        val = _mm512_loadunpackhi_ps(val, (float*)src+16);
    }

    //load non-continuous memory space, src+index[i]*scale, scale has to be 0, 2, 4, 8...
    void load(void *src, const vint &index, int scale){
        val = _mm_gather_ps(index.val, src, scale);
    }
    ...
    //store to continuous memory space (unaligned)
    void storeu(void *dst){
      _mm512_packstorelo_ps(dst, val);
      _mm512_packstorehi_ps((float*)dst+16, val);
    }
